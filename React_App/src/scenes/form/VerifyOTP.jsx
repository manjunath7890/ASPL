...
const VerifyOTP = ({ email: propEmail }) => {
  ...
  const handleFormSubmit = async (values) => {
    if (!email) {
      window.alert("Email is missing. Please request OTP again.");
      navigate("/forgot-password");
      return;
    }

    try {
      const response = await fetch(`${colors.palette[50]}/api/verify-otp`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        window.alert(errorData.message || "Invalid OTP");
      }
    } catch (error) {
        ...
        >
          {({
            ...
            <form onSubmit={handleSubmit}>
              ...
              {email && (
                <Typography sx={{ mb: 3.2, color: colors.palette[500], fontSize: "0.92rem" }}>
                  {email}
                </Typography>
              )}
